...
server <- function(input, output, session) {
  ...
  observeEvent(input$getResults_advanced, {
    userDataAdvanced <- data.frame(
      Age=as.numeric(input$userAge_advanced), 
      Weight=as.numeric(input$userWeight_advanced),
      Length=as.numeric(input$userHeight_advanced), 
      Sex=toString(getGender(input$userSex_advanced)),
      ...
      Airway.disease= "N", 
      Thyroid.Disease= str_sub(input$thyriodDisease, 1, 1),
      CHF= "N", 
